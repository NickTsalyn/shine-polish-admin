...
const Button = React.forwardRef<HTMLButtonElement, PropsButtton>((props, ref) => {
 ...
 switch (props.style) {
  ...
  case "delete-button":
   ...
   break;
  case "transparent-button-form":
   styles =
    " w-[20px] h-[20px] md:w-[32px] md:h-[32px] xl:w-[40px] xl:h-[40px] rounded-full bg-transparent border-none ";
   break;
  default:
   break;
 }
 const disabledStyles = "bg-gray-300 text-gray-500 cursor-not-allowed opacity-50";

 return (
  <button
   ref={ref}
   type={props.type}
   className={`${styles} ${props.disabled ? disabledStyles : ""}`}
